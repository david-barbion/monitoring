...
my $return_code = 0 ;
my $perfparse = "" ;
my $outlabel = "Mirapoint mailqueue: ";
my @oids= ("1.3.6.1.4.1.3246.2.2.1.4.3.1.0") ;

$result = $session->get_request(-varbindlist => \@oids) ;
my $mailqueue_size = $result->{"1.3.6.1.4.1.3246.2.2.1.4.3.1.0"} ;

$outlabel.="$mailqueue_size messages queued " ;
$perfparse .= "'$label'=".$mailqueue_size."$unit;" ;
	
if (defined($opt_c) && defined($opt_w)) {
    if ($mailqueue_size > $opt_w) {
	if ($mailqueue_size > $opt_c) {
	    $return_code = 2 ;
	}else {
	    $return_code = 1 ;
	}
