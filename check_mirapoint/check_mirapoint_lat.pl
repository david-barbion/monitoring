...
my $return_code = 0 ;
my $output ;
my $outlabel = "Mirapoint R/W Latency ";
my %oids= ( 	"RaidLatRead" ,	".1.3.6.1.4.1.3246.2.2.3.24.0",
		"RaidLatWrite",	".1.3.6.1.4.1.3246.2.2.3.25.0") ;
...
while(($label, $oid) = each(%oids)) {
	...
	if ($return_result > $opt_w) {
	    if ($return_result > $opt_c) {
		$return_code = 2 ;
	    }else {
		$return_code = 1 ;
	    }
